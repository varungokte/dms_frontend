...
function TableCollapsible(props:{topRow:[string,string][], bottomRow:[string|ReactElement,string][], content:ReactElement, searchString:string}){
    ...
    <Collapsible>
      <div className="bg-zinc-100 m-3 rounded-2xl">
        {props.searchString=="" || props.topRow[0][0]==props.searchString
          ?<CollapsibleTrigger className="w-full" onClick={()=>setChevronToggle(curr=>{return !curr})}>
            <Table>
              <TableBody>
                <TableRow className="border-none w-full">
                  {props.topRow.map((column, index:number)=>{
                ...
                <TableRow className="border-none">
                  {props.bottomRow.map((column,index:number)=>{
                    return <TableCell key={index} className={`${column[1]}`}>{column[0]}</TableCell>
                  })}
                </TableRow>
              </TableBody>
