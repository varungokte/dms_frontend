...
function PieChart(props:{title:string,parameters:{name:string, value:number}[]}) {
  ...
  return (
    <div className="p-5 flex flex-col items-center justify-around sm:px-7.5 col-span-12 rounded-sm border border-stroke bg-white px-5 pb-5 pt-7.5 shadow-default dark:border-strokedark dark:bg-boxdark xl:col-span-4">
      ...
      <div className="mb-2">
        <div id="chartThree" className="mx-auto flex justify-center">
          <ReactApexChart
            options={options}
            series={props.parameters.map(param=>param.value)}
            type="donut"
          />
        </div>
      </div>
            ...
            <div key={index} className="sm:w-1/2 w-full px-8">
              <div className="flex w-full items-center">
                ...
                <p className="flex w-full justify-between text-sm font-medium text-black dark:text-white">
                  ...
                  <span> {(param.value*100/totalValue).toFixed(2)}% </span>
                </p>
              </div>
            </div>
          )
