...
function PermissionsField (props:{index:number, fieldData:FormFieldAttributes, permissionPreset:FieldValues, setPermissionSet:Function, disabled:boolean}){
  try{
                      ...
                      {value=="documents"
                        ?Object.keys(permissionSet).map((subsection,subindex)=>{
                          ...
                          return <div key={index+"_"+subindex}>
                            ...
                            <RenderPermissions index={subindex} id={props.fieldData.id} sectionName={subsection} permissionSet={permissionSet[subsection]} setPermissionSet={props.setPermissionSet} category={section} disabled={props.disabled} multiple={props.fieldData.multiple} />
                            <br />
                          </div>
                        })
...
function RenderPermissions(props:{index:number|string, id:string, sectionName:string, category:string, permissionSet:any, setPermissionSet:Function,disabled?:boolean, multiple?:boolean}){
  ...
  useEffect(()=>console.log("render field",props.permissionSet),[props.permissionSet])
          ...
          onClick={()=>{
            try{
              ...
              props.setPermissionSet((curr:any)=>{
                ...
                if (props.multiple)
                  return [...curr];
                else{console.log("SENDING BACK",curr)
                  return {...curr};}
              }) 
            }
