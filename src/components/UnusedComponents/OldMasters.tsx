...
function Masters(props:{label:string, masterLists: FieldValues, idList:FieldValues, callMasterLists:Function}){
  ...
  const [selected, setSelected] = useState(Object.keys(props.masterLists)[0]);
      ...
      {props.masterLists
        ?Object.keys(props.masterLists).length!=0
          ?<div className="flex flex-row relative m-10">
            <TableSelect tableHeading="Category" tableValues={Object.keys(props.masterLists).filter(val=>Object.values(adminEnteredMasters).includes(val))} selected={selected} setSelected={setSelected} className={"w-[30%]"} textSize="large" />

            <div className="mx-20"></div>
            {newValues && newValues["V"]
              ?<div className="mr-28 w-[50%]">
                <Table className="rounded-2xl bg-white">
                  <DataTableHead headingRows={[selected]} headingClassNames={["text-2xl"]} />
                  <TableBody>
                    ...
                    <TableRow>
                      <TableCell>
                        {userPermissions["masters"].includes("edit") && <button onClick={createNewEntry}><AddCircleIcon sx={{color:"rgba(80, 65, 188, 1)"}} /></button>}
